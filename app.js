...
function speakWithYoudao(text, lang, rate) {
    ...
    audio.onerror = () => {
        _isSpeaking = false;
        _currentAudio = null;
        console.warn('有道 TTS 加载失败，降级到 speechSynthesis...');
        speakWithBrowser(text, lang, rate);
    };

    // 关键：必须在用户点击的同步上下文中设置 src 并调用 play()
    // 移动端浏览器要求 play() 在用户手势回调中调用
    // 浏览器会自动缓冲音频，加载完成后开始播放
    audio.src = url;
    ...
    if (playPromise) {
        playPromise.then(() => {
            ...
            // play() 被拒绝（可能是音频未加载完或浏览器限制）
            // 不立即降级到 speechSynthesis，等待音频加载后重试
            console.warn('有道 TTS play() 暂时被拒绝，等待加载后重试:', err.name || err);
            const retryPlay = () => {
                if (_currentAudio !== audio) return;
                audio.play().then(() => {
                    _isSpeaking = true;
                }).catch(err2 => {
                    console.warn('有道 TTS 重试失败，降级到 speechSynthesis:', err2.name || err2);
                    _currentAudio = null;
                    _isSpeaking = false;
                    speakWithBrowser(text, lang, rate);
                });
            };
            // 监听 canplay 事件（音频可播放时重试）
            audio.addEventListener('canplay', retryPlay, { once: true });
            // 超时兜底：3 秒后强制重试
            setTimeout(() => {
                audio.removeEventListener('canplay', retryPlay);
                if (_currentAudio === audio && !_isSpeaking) {
                    retryPlay();
                }
            }, 3000);
        });
...
function renderEnglishSentences() {
    ...
    display.innerHTML = sentences.map((s, i) => {
        ...
        const zh = typeof s === 'object' ? s.zh : '';
        return `
        <div class="en-sentence-card">
            <div class="en-text">${esc(en)}</div>
            ${zh ? `<div class="en-zh">${esc(zh)}</div>` : ''}
        </div>
    `}).join('');
}
...
const sentenceLibrary = [
    { sentences: [{ text: "The only way to do great work is to love what you do.", zh: "做伟大工作的唯一途径就是热爱你所做的事。", source: "Steve Jobs" }, { text: "Dream big and dare to fail.", zh: "大胆梦想，敢于失败。", source: "Norman Vaughan" }], goodWords: [{ word: "great", zh: "伟大的", example: "She did a great job on her English test." }, { word: "dream", zh: "梦想", example: "My dream is to travel around the world." }, { word: "dare", zh: "敢于", example: "I dare to speak English in front of the class." }] },
    { sentences: [{ text: "Knowledge is power.", zh: "知识就是力量。", source: "Francis Bacon" }, { text: "Learning is a lifelong journey.", zh: "学习是一段终生的旅程。", source: "Unknown" }], goodWords: [{ word: "knowledge", zh: "知识", example: "We can get knowledge from books and teachers." }, { word: "learning", zh: "学习", example: "English learning requires practice every day." }, { word: "journey", zh: "旅程", example: "Our life is a wonderful journey full of surprises." }] },
    { sentences: [{ text: "Where there is a will, there is a way.", zh: "有志者事竟成。", source: "Proverb" }, { text: "Practice makes perfect.", zh: "熟能生巧。", source: "Proverb" }], goodWords: [{ word: "will", zh: "意志", example: "She has a strong will to learn English well." }, { word: "practice", zh: "练习", example: "Daily practice helps improve your pronunciation." }, { word: "perfect", zh: "完美的", example: "Nobody is perfect, but we can always improve." }] },
    { sentences: [{ text: "The best preparation for tomorrow is doing your best today.", zh: "对明天最好的准备就是今天做到最好。", source: "H. Jackson Brown Jr." }, { text: "You are never too old to learn.", zh: "活到老学到老。", source: "Proverb" }], goodWords: [{ word: "preparation", zh: "准备", example: "Good preparation is the key to success." }, { word: "tomorrow", zh: "明天", example: "I will finish my homework tomorrow morning." }, { word: "learn", zh: "学习", example: "Children learn new things very quickly." }] },
    { sentences: [{ text: "Every child is an artist. The problem is how to remain an artist once we grow up.", zh: "每个孩子都是艺术家。问题是如何在长大后依然保持艺术家的心。", source: "Pablo Picasso" }, { text: "Creativity is intelligence having fun.", zh: "创造力是智力在玩耍。", source: "Albert Einstein" }], goodWords: [{ word: "artist", zh: "艺术家", example: "My sister wants to be an artist in the future." }, { word: "creativity", zh: "创造力", example: "Drawing helps develop children's creativity." }, { word: "intelligence", zh: "智力", example: "Reading books can increase your intelligence." }] }
];
...
function renderSentenceModule() {
    ...
    sBox.innerHTML = sentences.map((item, i) => `
        <div class="sentence-card-box">
            ...
            ${item.zh ? `<div class="sentence-zh">${esc(item.zh)}</div>` : ''}
    ...
    wBox.innerHTML = goodWords.map((item, i) => `
        <div class="word-item-box">
            ...
            ${item.zh ? `<span class="word-zh">${esc(item.zh)}</span>` : ''}

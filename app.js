 ...
 *
 * 兼容性说明：
 * - Chrome 桌面：Google 云英语语音在国内不可用，需用本地语音兜底
 * - Edge：内置 Microsoft 语音，一切正常
 * - 手机浏览器（华为/小米/Safari）：必须在用户手势同步调用栈中 speak()
 *
 * 解决方案：
 * 1. findBestVoice() 优先本地语音，找不到英语语音时用中文兜底
 * 2. 优先同步调用 speak()（保持用户手势上下文，手机浏览器必须）
 * 3. cancel() 后直接 speak()，如果 500ms 后仍未启动则重试
 * 4. 每 10 秒 pause/resume 保活，防止 Chrome 长文本朗读中断
 * ================================================================ */
...
/**
 * 查找最佳语音
 * 优先级：本地精确匹配 > 本地前缀匹配 > 云端精确匹配 > 中文兜底 > 任意兜底
 */
function findBestVoice(voices, lang) {
    ...
    // 英语：优先本地 Microsoft 语音
    if (prefix === 'en') {
        // 1. 本地 Microsoft 英语语音（最可靠）
        let v = voices.find(v => v.localService && v.lang.toLowerCase().startsWith('en') && /microsoft/i.test(v.name));
        if (v) return v;
        // 2. 任意本地英语语音
        v = voices.find(v => v.localService && v.lang.toLowerCase().startsWith('en'));
        if (v) return v;
        // 3. 云端英语语音（Google 等，可能在国内网络不可用）
        v = voices.find(v => v.lang.toLowerCase().startsWith('en'));
        if (v) return v;
        // 4. 中文语音兜底（比完全无声好）
        v = voices.find(v => v.localService && v.lang.toLowerCase().startsWith('zh'));
        if (v) return v;
        // 5. 任意本地语音兜底
        v = voices.find(v => v.localService);
        return v || null;
    }

    // 中文：优先本地
    if (prefix === 'zh') {
        let v = voices.find(v => v.localService && v.lang.toLowerCase().startsWith('zh'));
        if (v) return v;
    ...
    // 通用匹配
    let v = voices.find(v => v.localService && v.lang.toLowerCase() === langLower);
    if (v) return v;
    v = voices.find(v => v.localService && v.lang.toLowerCase().startsWith(prefix));
    if (v) return v;
    v = voices.find(v => v.lang.toLowerCase() === langLower);
    if (v) return v;
 ...
 * 内部朗读实现
 */
function _doSpeak(cleanText, lang, rate, isRetry) {
    ...
    utter.onerror = (e) => {
        ...
        if (e.error !== 'interrupted' && e.error !== 'canceled') {
            console.warn('语音朗读错误:', e.error);
        }
    };

    // 如果不是重试，先 cancel 再 speak
    if (!isRetry) {
...
function speakText(text, lang = 'en-US', rate = 0.9) {
    ...
    // 【关键】同步调用 speak()，保持用户手势上下文
    // 手机浏览器（华为/小米/Safari）要求 speak() 必须在 click 事件的同步调用栈中
    _doSpeak(cleanText, lang, rate, false);

    // 【兜底】如果 500ms 后仍未开始朗读，用新 utterance 重试一次
    // 适用于：Chrome 首次调用 TTS 引擎未激活、cancel+speak 静默等情况
    setTimeout(() => {
        if (!window.speechSynthesis.speaking && !window.speechSynthesis.pending) {
            console.log('朗读未启动，尝试重试...');
            _doSpeak(cleanText, lang, rate, true);
        }
    }, 500);
}

// 预加载语音列表（Chrome 异步加载，必须监听 voiceschanged）
...
function switchGrade(grade) {
    ...
    if (textbookOption) textbookOption.textContent = '课本（' + gradeLabel + '）';
